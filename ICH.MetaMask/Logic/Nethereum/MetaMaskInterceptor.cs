...
namespace ICH.MetaMask.Logic.Nethereum
{
  public class MetaMaskInterceptor : RequestInterceptor
  {
    ...
    public MetaMaskInterceptor(MetaMaskInterop metamaskInterop, MetaMaskHostProvider metamaskHostProvider)
    {
    ...
    public override async Task<object> InterceptSendRequestAsync<T>(
        ...
        Task<T>> interceptedSendRequestAsync,
        string method,
        string route = null,
    ...
    protected static void HandleRpcError(RpcResponseMessage response)
    {
      if (response.HasError)
        throw new RpcResponseException(
          new global::Nethereum.JsonRpc.Client.RpcError(
            response.Error.Code,
            response.Error.Message,
            response.Error.Data));
    }

    private static T ConvertResponse<T>(RpcResponseMessage response)
    {
